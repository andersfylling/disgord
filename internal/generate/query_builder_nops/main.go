...
import (
	"bytes"
	"fmt"
	"github.com/andersfylling/disgord/internal/generate/querybuilderparser"
	"go/ast"
	"go/format"
	goparser "go/parser"
	"go/token"
...
func main() {
	...
	for i := range files {
		...
		querybuilderparser.Parse(file, func(i *ast.TypeSpec) {
	...
	makeFile(enforcers["Copier"], "internal/generate/inter/NopQueryBuilders.gotpl", "nop_query_builders_gen.go")
}
