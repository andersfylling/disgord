...
func main() {
	...
	// TestGetCurrentUser
	func() {
		_, err = c.GetCurrentUser().IgnoreCache().Execute()
		if err != nil {
			panic(err)
		}
	}()
	...
	func() {
		...
		user, err := c.GetUser(userID).IgnoreCache().Execute()
		if err != nil {
			panic(err)
	...
	// TestListGuildEmojis
	func() {
		emojis, err := c.GetGuildEmojis(keys.GuildDefault).Execute()
	...
	func() {
		list, err := c.GetVoiceRegions().Execute()
		if err != nil {
			panic(err)
		} else if len(list) == 0 {
